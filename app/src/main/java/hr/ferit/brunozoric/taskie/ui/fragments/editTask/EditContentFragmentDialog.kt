...
class EditContentFragmentDialog: DialogFragment() {
        ...
        saveEditAction.setOnClickListener {
            interactor.editNote(
                EditTaskRequest(
                    id = task.id,
                    title = task.title,
                    content = editTaskInput.text.toString(),
    ...
    private fun initUi() {
        editTaskHeading.text = getString(R.string.edit_description)
        editTaskInput.setText(task.content)
    }

    companion object{
        fun newInstance(): EditContentFragmentDialog {
            return EditContentFragmentDialog()
        }
    }
}